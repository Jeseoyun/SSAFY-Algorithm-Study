...
def find_the_longest(row, col, K, length):
    ...
    for x, y in zip(dx, dy):
        ...
        if 0<= nx < N and 0 <= ny < N and not visited[nx][ny]:
            ...
            # 다음 이동하는 값이 현재 값과 같다면?
            elif road[nx][ny] == road[row][col] and K:
                temp = road[nx][ny]
                #다음 점을 현재 점보다 1만 작게 만들면 됨
                road[nx][ny] = road[nx][ny] - 1
                find_the_longest(nx, ny, 0, length + 1)
                #원상복귀
                road[nx][ny] = temp
            # 다음 이동하는 값이 현재 값보다 적다면?
            elif road[nx][ny] > road[row][col] and K > (road[nx][ny] - road[row][col]):
                temp = road[nx][ny]
                # 다음 점을 현재 점보다 1만 작게 만들면 됨
                road[nx][ny] = road[nx][ny] - 1
                find_the_longest(nx, ny, 0, length + 1)
