...
- 등산 기준, 현재 위치 +1 이 되도록 깍으면 최고. 단, K조건 만족할 경우 
- 하산 기준, 현재 위치 -1 이 되도록 깍으면 최고, 단, K조건 만족할 경우


# 만약, 질문에 최고점에서 시작해라는 말 같은 힌트가 없고, 그냥 최장 경로를 구해라고 한다면?
# 최고점에서 할지, 최저점에서 할지 말이 없었다면?
...
- 이외에도, 문제에서 출발점이 최고높이라고 명시적으로 알려줌

2. DFS로 접근
- 최단거리를 찾는 문제가 아니기 때문에 반드시 BFS 안써도 된다고 생각함
- 오히려, 쿠폰 조건이나 변형된 산 높이등 재귀적으로 호출하기 위해 DFS가 낫다고 생각함
